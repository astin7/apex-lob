...
Bash
git clone [https://github.com/astin7/apex-lob.git](https://github.com/astin7/apex-lob.git)
cd apex-lob
Generate the build system and force clean compile optimizations:

PowerShell
cmake -B build -S .
cmake --build build --config Release --clean-first
Route the built binary artifact directly into the runtime suite:

PowerShell
Copy-Item .\build\Release\apex_lob*.pyd .\python\
Launch the live terminal trading grid:

PowerShell
cd python
python terminal_ui.py
Run the hardware nanosecond latency benchmarking script:
