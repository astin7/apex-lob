...
public:
    ...
    void process_order(const RawOrder& incoming_order) {
        ...
        if (incoming_order.side == 'B') {
            // Crossing Logic: Check if the buyer's price is >= the lowest seller
            while (remaining_qty > 0 && best_ask <= price) {
                ...
                // Move the best ask pointer up if liquidity at this price is gone
                if (ask_level.total_volume == 0) {
                    best_ask++;
            ...
            // Rest remaining volume in the book
            if (remaining_qty > 0) {
            ...
            while (remaining_qty > 0 && best_bid >= price && best_bid > 0) {
                ...
                // Move the best bid pointer down if liquidity at this price is gone
                if (bid_level.total_volume == 0) {
                    best_bid--;
                }
            }
